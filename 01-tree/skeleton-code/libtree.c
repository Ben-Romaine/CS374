...
print_path_info(struct fileinfo finfo)
{
  ...
  if (opts.user) {
    /*  Hint: getpwuid(3) */
    if (printf("%c%s", sep, mode_string(finfo.st.st_uid)) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.group) {
    /*  Hint: getgrgid(3) */
    if (printf("%c%s", sep, mode_string(finfo.st.st_gid)) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.size) {
    /*  Hint: stat.h(0p) */
    if (printf("%c%jd", sep, (finfo.st.st_size)-12) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (sep != '[')
